...
public:
	...
	BinSTree(TreeNode<T>* root = NULL)
	{
		_root = root;
	}

	void SetRoot(TreeNode<T>* root = NULL)
	{
		_root = root;
	}
	...
	void INSERT(T item)
	{
		...
		while (temp != NULL || temp->_data != item)
		{
			if (temp->_data > item)
			{
				temp = temp->_left;
			...
			else
			{
				...
				else if (temp->_right != NULL && temp->_left != NULL)
				{
					if (temp->_right->_left == NULL)
					{
						...
						temp = new TreeNode<T>(temp->_right->_data, temp->_left, NULL);
						tp.Free();
					}
					else
					{
						...
						temp = new TreeNode<T>(tp->_data, temp->_left, temp->_right);
						tp->Free();
					}
				}
				// Do have 1 children;
	...
	vector<T> GetData()
	{
		vector<T> res(Pow(2, _root->GetDepth() + 1));

		vector<TreeNode<T>*> nodes = this->VisitWidth();
	...
	void PrintTreeH(TreeNode<T>* node = NULL, int indent = 8, int level = 0)
	{
		if (node != NULL)
		{
